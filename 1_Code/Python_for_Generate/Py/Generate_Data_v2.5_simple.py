...
def evaluate_and_print(alaph1, alaph2, v_noise, ptw_weight):
    ...
    print(f"\n参数: alaph1={alaph1}, alaph2={alaph2}, v_noise={v_noise}, ptw_weight={ptw_weight}")
    print(f"  整体: SPE={overall_spe:.1f}ms, RT={overall_rt:.0f}ms, Acc={overall_acc:.1f}%")
    print(f"  各组SPE: ", end="")
    for g in range(1, 7):
        if g in group_spe:
            print(f"G{g}:{group_spe[g]:.0f}({TARGET_SPE[g]:.0f}) ", end="")
    print()
...
for alaph1, alaph2, v_noise, ptw_weight in test_params:
    overall_spe, group_spe, overall_rt, overall_acc = evaluate_and_print(alaph1, alaph2, v_noise, ptw_weight)
    ...
    if total_error < best_error:
        ...
        best_params = (alaph1, alaph2, v_noise, ptw_weight)

print("\n" + "=" * 70)
print(f"最优参数: alaph1={best_params[0]}, alaph2={best_params[1]}, v_noise={best_params[2]}, ptw_weight={best_params[3]}")
print(f"最小误差: {best_error:.2f}")
print("=" * 70)

print("\n使用最优参数生成最终数据...")
alaph1, alaph2, v_noise, ptw_weight = best_params
a_noise = v_noise * 0.75
...
df_final, _ = generate_dataset_v2_5(
    ...
    alaph1=alaph1, alaph2=alaph2,
    v_noise=v_noise, a_noise=a_noise,
    ptw_weight=ptw_weight, seed=123
)
