...
def generate_dataset_v2_5_v2(n_subjects_per_group=8, trials_per_sub=60, 
                             base_v=1.0, alaph_self=0.8, alaph_stranger=1.0,
                             v_noise=0.4, a_noise=0.3, w_gp=0.5, 
                             ptw_v_power=1.5, seed=42):
    np.random.seed(seed)
    ...
    for cond in EXPERIMENT_CONDITIONS:
        ...
        ptw_factor = ((P + 1) / 121.0) ** 0.5 * (T / 500.0) ** 0.5 * (W / 1500.0) ** 0.5
        ptw_factor = ptw_factor ** ptw_v_power
...
def evaluate_params(alaph_self, alaph_stranger, v_noise, ptw_v_power):
    a_noise = v_noise * 0.75
    
    df, _ = generate_dataset_v2_5_v2(
        n_subjects_per_group=6,
        trials_per_sub=40,
        base_v=1.0,
        alaph_self=alaph_self,
        alaph_stranger=alaph_stranger,
        v_noise=v_noise,
        a_noise=a_noise,
        ptw_v_power=ptw_v_power,
        seed=42
    )
    ...
    return overall_spe, group_spe, overall_rt, overall_acc

print("=" * 70)
print("V2.5 参数搜索 - 新模型结构 (简化搜索)")
print("=" * 70)
...
test_params = []
for alaph_self in [0.5, 0.6, 0.7, 0.8]:
    for alaph_stranger in [0.8, 0.9, 1.0, 1.1]:
        for v_noise in [0.3, 0.4]:
            for ptw_v_power in [1.5, 2.0, 2.5]:
                test_params.append((alaph_self, alaph_stranger, v_noise, ptw_v_power))

print(f"总共测试 {len(test_params)} 组参数...")

for i, (alaph_self, alaph_stranger, v_noise, ptw_v_power) in enumerate(test_params):
    overall_spe, group_spe, overall_rt, overall_acc = evaluate_params(
        alaph_self, alaph_stranger, v_noise, ptw_v_power
    )
    
    spe_error = sum((group_spe.get(g, 0) - TARGET_SPE[g]) ** 2 for g in range(1, 7))
    overall_error = 0.3 * (overall_spe + 14.2) ** 2 + 0.001 * (overall_rt - 563) ** 2 + 0.5 * (overall_acc - 67.6) ** 2
    total_error = spe_error + overall_error
    
    if total_error < best_error:
        best_error = total_error
        best_params = (alaph_self, alaph_stranger, v_noise, ptw_v_power)
        print(f"[{i+1}/{len(test_params)}] 新最优! alaph_self={alaph_self}, alaph_stranger={alaph_stranger}, v_noise={v_noise}, ptw_power={ptw_v_power}")
        print(f"  SPE: {overall_spe:.1f}ms, RT: {overall_rt:.0f}ms, Acc: {overall_acc:.1f}%")
        for g in range(1, 7):
            print(f"    G{g}:{group_spe.get(g,0):.0f}({TARGET_SPE[g]:.0f}) ", end="")
        print()

print("\n" + "=" * 70)
print(f"最优参数: alaph_self={best_params[0]}, alaph_stranger={best_params[1]}, v_noise={best_params[2]}, ptw_power={best_params[3]}")
print(f"最小误差: {best_error:.2f}")
print("=" * 70)

print("\n使用最优参数生成最终数据...")
alaph_self, alaph_stranger, v_noise, ptw_v_power = best_params
a_noise = v_noise * 0.75

df_final, _ = generate_dataset_v2_5_v2(
