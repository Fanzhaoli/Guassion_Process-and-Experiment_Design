...
for csv_path in csv_files:
    ...
    try:
        ...
        if traces_simple:
            ...
            print("  get_traces() 返回空，迹线未保存")
    except Exception as e:
        print(f"  迹线保存失败: {e}")
        print(f"  仅统计文件可用")

    n_valid = (df["omission"] == 0).sum()
    resp_matching_pct = df.loc[df["omission"] == 0, "response"].mean() if n_valid > 0 else np.nan
    ...
    print(f"  有效试次: {n_valid}, 判断Matching比例: {resp_matching_pct:.3f}")
