...
print("=" * 60)
print("Step 3: 提取 HDDM 参数后验分布 (NonMatching模型)")
print("=" * 60)
...
if "SPE_v" in df_params.columns:
    ...
    ax.set_title("SPE in Drift Rate by Group (All Trials)")
...
if "v_self_mean" in df_params.columns:
    ...
    ax.set_title("Drift Rate by Condition (All Trials)")
    ax.legend()
...
if "a_mean" in df_params.columns:
    ...
    ax.set_title("Decision Boundary by Group (All Trials)")
...
if "t_mean" in df_params.columns:
    ...
    ax.set_title("Nondecision Time by Group (All Trials)")
...
if "SPE_v" in df_params.columns and "P" in df_params.columns:
    ...
    for i, (var, xlabel) in enumerate([
        ...
        ax.set_ylabel("SPE_v")
        ax.set_title(f"SPE_v vs {xlabel} (All Trials)")
    plt.tight_layout()
